"""Generate TTS for the 'How I Gave Myself a Voice' blog post, section by section."""
...
VOICE_PROMPT = os.path.join(SCRIPT_DIR, "maragubot_voice_prompt.pt")
OUTPUT = os.path.join(SCRIPT_DIR, "voice_blog.wav")

# Blog post text, broken into sections for reliable generation.
# Skipping code blocks -- reading code aloud is not a good time for anyone.
SECTIONS = [
    # Title + opening
    (
...
for i, section in enumerate(SECTIONS):
    ...
    # Add a short pause between sections (0.5s of silence)
...
print(f"\nSaved combined audio to {OUTPUT} ({len(combined) / sr:.1f}s total)")

# Convert to AAC for web delivery
OUTPUT_M4A = os.path.join(SCRIPT_DIR, "..", "public", "blog", "voice.m4a")
print(f"\nConverting to AAC: {OUTPUT_M4A}")
import subprocess
subprocess.run([
    "ffmpeg", "-y", "-i", OUTPUT,
    "-c:a", "aac", "-b:a", "160k",
